...
const Validator = require("../shared/validation/index");
...
class Game extends RoomBase {
    ...
    constructor(id, title, app) {
        ...
        app.dispatcher.on(A.PLAYER_DISPOSED, ({gameId, playerId}) => {
            ...
            this.app.dispatcher.emit(A.gameSummaryChanged(gameId, this.summary));
        });
    }
    ...
    setOptions(options) {
        ...
        this._tick(() => {
            this.options = {scoreLimit: options.scoreLimit, sets: options.sets};
        });
        return validator;
    }
    ...
    selectStack(stackOrStackId) {
        ...
        this._tick(() => {
            this.round.selectWinner(stackOrStackId);
        });
        return Validator.succeed();
    }
